...
import { GraduationCap, Library, User, Sparkles, BookOpen, ClipboardCheck, ChevronDown, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function SideMenu({ 
  ...
  const [isMinimized, setIsMinimized] = React.useState(() => {
    try {
      const saved = localStorage.getItem("teachsam-sidebar-minimized");
      return saved === "true";
    } catch {
      return false;
    }
  ...
  const toggleMinimize = () => {
    setIsMinimized(prev => {
      const next = !prev;
      try {
        localStorage.setItem("teachsam-sidebar-minimized", String(next));
      } catch (err) {
        console.error(err);
      }
      return next;
  ...
  return (
    <aside className={`hidden lg:flex transition-all duration-300 ${isMinimized ? "w-20" : "w-64"} border-r border-zinc-200 flex-col h-screen sticky top-0 shrink-0 select-none ${isDark ? "bg-[#121318] border-zinc-800 text-white" : "bg-white text-black"}`}>
      {/* Brand Header */}
      <div className={`p-5 border-b border-zinc-200 flex flex-col items-center justify-center gap-2 relative ${isDark ? "bg-[#181a22] border-zinc-850" : "bg-[#fcfcfd]"}`}>
        
        {/* Minimize / Expand Toggle Button on the border line */}
        <button
          onClick={toggleMinimize}
          title={isMinimized ? "Expand Sidebar" : "Minimize Sidebar"}
          className={`absolute -right-3.5 top-1/2 -translate-y-1/2 z-50 p-1.5 border-2 border-black rounded-lg transition-all hover:scale-105 active:scale-95 shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,1)] hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer flex items-center justify-center ${isDark ? "bg-zinc-900 border-zinc-700 text-white" : "bg-white border-black text-black"}`}
        >
        ...
        <button 
          ...
          title={isMinimized ? `Switch Buddy (Current: ${activeChar.name})` : undefined}
          className={`group relative flex flex-col items-center justify-center focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-zinc-400 rounded-2xl p-1.5 transition-all hover:bg-zinc-100/30 dark:hover:bg-zinc-800/30 cursor-pointer text-center ${isMinimized ? "w-12 h-12" : "w-full"}`}
        >
          <div className="relative">
            <AvatarMascot expression={expression} size={isMinimized ? "sm" : "md"} character={activeCharacterId} />
            <div className="absolute -bottom-1 -right-1 bg-white dark:bg-[#181a22] border border-black rounded-full p-0.5 shadow-sm group-hover:scale-110 transition-transform">
              <ChevronDown className="w-2.5 h-2.5 text-black dark:text-white" />
            </div>
          </div>
          {!isMinimized && (
        ...
        {dropdownOpen && (
          <>
            ...
            <div className={`absolute top-[92%] ${isMinimized ? "left-2 w-56" : "left-4 right-4 w-[calc(100%-2rem)]"} z-50 border-4 border-black rounded-2xl p-2 shadow-[4px_4px_0px_0px_#000] text-left select-none text-black ${isDark ? "bg-[#181922] text-white" : "bg-white text-black"}`}>
      ...
      {/* Navigation Options */}
      <nav className={`flex-1 ${isMinimized ? "p-2" : "p-4"} space-y-4 overflow-y-auto`}>
            ...
            return (
              <li key={tab.id}>
                <button
                  ...
                  title={tab.label}
                  className={`w-full flex items-center ${isMinimized ? "justify-center p-3" : "gap-3 px-4 py-3"} rounded-xl font-bold text-xs uppercase tracking-wider transition-all border select-none cursor-pointer
                    ${isActive 
        ...
        <button 
          ...
          title="Need help or got a complaint? Tell Ayo!"
          className={`flex items-center justify-center transition-all cursor-pointer select-none border-2 border-black rounded-xl hover:scale-105 active:scale-95 text-left
            ${isMinimized 
              ? `w-11 h-11 mx-auto ${isDark ? "bg-[#181920] border-zinc-700" : "bg-amber-100 hover:bg-amber-50"}` 
              : `p-4 mt-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 w-full ${isDark ? "bg-[#181920]/80 border-zinc-805 hover:bg-[#232530]" : "bg-gradient-to-br from-amber-50/50 to-amber-100/30 border-amber-200/50 hover:bg-amber-50"}`
            }`}
        >
          {isMinimized ? (
            <div className="relative flex items-center justify-center">
              <span className="relative flex h-1.5 w-1.5 absolute -top-1 -right-1">
